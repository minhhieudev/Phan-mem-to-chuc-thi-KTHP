...
// POST multiple users with update if username exists
export const POST = async (req) => {
  try {
    await connectToDB();
    ...
    const processedUsers = await Promise.all(
      users.map(async (user) => {
        const [username, email, khoa, role] = user; // Destructure user array

        // Tìm và cập nhật nếu người dùng tồn tại, nếu không thì tạo mới
        const updatedUser = await User.findOneAndUpdate(
          { username }, // Tìm người dùng theo username
          {
            email, // Cập nhật các trường thông tin
            khoa,
            role,
          },
          { new: true, upsert: true } // Nếu không tìm thấy thì tạo mới
